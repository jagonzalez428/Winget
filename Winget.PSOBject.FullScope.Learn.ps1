...
if ($WingetInfo[-1] -ne "No package found matching input criteria.") {
    ...
    $Version = if (($WingetInfo | Select-String "Version: *")) { ((($WingetInfo | Select-String "Version: *").ToString()).Trim("Version:")).Trim() }
    $Publisher = if (($WingetInfo | Select-String "Publisher: *")) { ((($WingetInfo | Select-String "Publisher: *").ToString()).Trim("Publisher:")).Trim() }
    $PublisherURL = ( -split ($WingetInfo | Select-String "Publisher URL:*"))[-1]
    $PublisherSupportURL = ( -split ($WingetInfo | Select-String "Publisher Support URL:*"))[-1]
    $Author = if (($WingetInfo | Select-String "Author: *")) { ((($WingetInfo | Select-String "Author: *").ToString()).Trim("Author:")).Trim() }
    $Moniker = if (($WingetInfo | Select-String "Moniker: *")) { ((($WingetInfo | Select-String "Moniker: *").ToString()).Trim("Moniker:")).Trim() }
    if (($WingetInfo | Select-String "Description: *") -and ($WingetInfo | Select-String "Homepage: *")) {
        $Description = "(?<=Description:)(.*?)(?=Homepage:)"
        $SelectedText = [regex]::Matches($WingetInfo, $Description)
        $Description = $SelectedText[0].Value.Trim()
    }
    $Homepage = ( -split ($WingetInfo | Select-String "Homepage:*"))[-1]
    $License = ((($WingetInfo | Select-String "License: *").ToString()).Trim("License:")).Trim()
    $LicenseURL = ( -split ($WingetInfo | Select-String "License URL:*"))[-1]
    $PrivacyURL = ( -split ($WingetInfo | Select-String "Privacy URL:*"))[-1]
    $Copyright = ((($WingetInfo | Select-String "Copyright: *").ToString()).Trim("Copyright:")).Trim()
    ...
    $PurchaseURL = ( -split ($WingetInfo | Select-String "Purchase URL:*"))[-1]
    ...
    $Tags = (-split $Tags)
    $InstallerType = ( -split ($WingetInfo | Select-String "Installer Type:*"))[-1]
    $InstallerURL = ( -split ($WingetInfo | Select-String "Installer URL:*"))[-1]
    $InstallerSHA256 = ( -split ($WingetInfo | Select-String "Installer SHA256:*"))[-1]
    ...
    if ($InstallerType) { $MachineScope = $true } else { $MachineScope = $false }
    if ($InstallerTypeUser) { $UserScope = $true } else { $UserScope = $false }

    # Generate PSObject
    $Winget = @()
    $Winget += [pscustomobject]@{
        Application         = $WingetApp
        WingetID            = $WingetID
        Installed           = $Installed
        ...
        UserScope           = $UserScope
        UserInstallerType   = $InstallerTypeUser
        UserInstallerURL    = $InstallerURLUser
        UserInstallerSHA256 = $InstallerSHA256User
    }
    # Result
    $Winget | Format-List
